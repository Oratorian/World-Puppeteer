...
```bash
.claude/skills/comfyui-image-gen/cli.exe --workflow anima.json --out images/generated/<name>.png --prompt "<prompt>"
```
...
```bash
python cli.py --workflow anima.json --out images/generated/<name>.png --prompt "<prompt>"
```
...
| `--workflow` | No | Workflow file: a name resolved inside the workflows folder, or a path. Default `base_workflow.json`. **`anima.json` takes natural language; `illustrious.json` takes Danbooru-style comma tags.** Must be **Export (API)** format. |
...
5. Assemble the prompt for `anima.json` (natural language). Lead with the **style**, then the curated appearance:
...
```
   ...
   Keep the curated appearance to visual facts only (build, hair, eyes, distinctive features, clothing + 1–2 signature items, primary weapon). Name the **style explicitly** — `anima.json` will not infer one. For a flat-anime look instead, swap the style clause; for `illustrious.json`, rewrite the whole thing as Danbooru tags.
...
7. Run cli.exe with `--workflow anima.json --out images/generated/[name-lowercase]-000.png --prompt "<prompt>"`. Add `--width 832 --height 1216` to force portrait size. Use `--workflow illustrious.json` only with a tag-style prompt. For each retry/variant, increment the `--out` suffix (`-001`, `-002`, …) yourself — cli.exe does not auto-increment.
